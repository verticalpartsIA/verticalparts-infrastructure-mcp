...
- o servidor MCP foi reiniciado e homologado com **49 tools** via `tools/list` (ver seção 4B para o catálogo atualizado de 2026-09-19, sessão 2: **62 tools**);
...
### 4B. Auditoria de segurança e expansão de tools — 2026-09-19 (sessão 2)

Uma segunda auditoria no mesmo dia, com break-glass habilitado sob confirmação por comando, encontrou e corrigiu o seguinte no host:

- **Firewall estava totalmente inativo** (`ufw status: inactive`, `iptables -L INPUT` com policy `ACCEPT` e nenhuma regra). Corrigido: `ufw` ativo, `default deny incoming`, liberado apenas `22/tcp`, `80/tcp`, `443/tcp` (+ IPv6). Qualquer porta nova exposta a `0.0.0.0` deve ser avaliada antes de liberar no firewall — não presumir que abrir a porta no processo basta.
- **Tor (`tor@default.service`) estava configurado com `SocksPort 0.0.0.0:9050`** (padrão do Tor é `127.0.0.1`) e tinha evidência real de abuso como proxy aberto (~15 mil conexões, ~360MB de tráfego nos dias antes da correção). **Cloudflare WARP (`warp-svc.service`)** estava instalado e ativo, mas nunca configurado (nunca aceitou os termos de serviço). Nenhum script/serviço conhecido dependia de nenhum dos dois. Ambos foram **parados e desabilitados** (`systemctl disable`, pacotes mantidos, reversível).
- **`hermes-agent-god7`** (container Docker do template Hostinger — agente de terminal remoto via hPanel, usa a imagem `ghcr.io/hostinger/hvps-hermes-agent`, **não é** o "Hermes AI Agent" da NousResearch documentado em `/root/vp-automations-hub/`) tinha só uma sessão de uso real registrada (2026-06-04) e o Traefik do host não conseguia rotear tráfego externo até ele (sem porta mapeada ao host — Nginx já ocupa 80/443). Removido por decisão do operador, com backup completo em `/root/hermes-agent-god7-backup-*.tar.gz`.
- **`mcp.vpsistema.com`** tem um segundo `location` (catch-all `/`, fora de `/omie/`) que faz proxy para `vpprd-mcp` (porta 3100, servidor MCP "vpprd-browser"). Esse `location` não tinha checagem de `X-API-Key` — foi adicionada, no mesmo padrão de `infra-mcp.vpsistema.com`/`whatsapp-mcp.vpsistema.com`. Chave nova em `/root/vpprd-mcp-auth-token`. Importante: o app `vpprd-mcp` já tinha autenticação própria via Bearer token internamente, então a exposição real era menor do que pareceu à primeira vista — a lacuna era só na camada de gateway.
- **Catálogo de tools expandido de 49 para 62** (ver `01_RAG-009` para lista completa): `firewall_status/allow/delete_rule`, `service_enable/disable`, `docker_network_ls/rm`, `docker_volume_ls/rm`, `docker_compose_action` (+ação `down`, `DESTRUCTIVE`), `infra_listening_ports`, `infra_pm2_list`, `infra_cron_list`, `file_delete`. PR: `verticalpartsIA/verticalparts-infrastructure-mcp#1`.
- **Descoberta de uma pilha de automação inteira não documentada** rodando na VPS além do já conhecido: `evolution-api` (backend WhatsApp da instância `pv360`/posvenda360), `n8n` + `vp-infra` (Postgres/Redis compartilhados), `traefik` (reverse proxy do template Hostinger, uso real limitado), `stt-service`, `telegram-claude`, `vpprd-mcp`, além de crons de negócio (`bordero`, `vpclick-cobranca`, `sac-backfill-diario`, `cron-handoffs`) e checkouts/projetos soltos (`resolve-360`, `vp-bot` dormente, `vp-automations-hub`, `vpprd-assistente`, `designsystem`). Tudo isso foi registrado em `config/projects.yaml` do runtime — **este é o inventário autoritativo para essa pilha**, os documentos canônicos não duplicam a lista granular.
- **Gotcha de auto-atualização**: `/opt/verticalparts-infrastructure-mcp` é `root:root`. As tools estruturadas `git_pull`/`file_write`/`env_set` rodam via SSH como usuário `infra-mcp` **sem sudo** — elas falham com `Permission denied` nesse diretório mesmo com break-glass habilitado, porque break-glass é uma tool separada (`infra_exec_command`, que roda com `sudo`). Para atualizar o próprio MCP, use o fluxo do `05_RUNBOOK` PARTE G, que já contempla isso (`git config --global --add safe.directory` + comandos via break-glass com `sudo git`). Isso é intencional, não regressão: o usuário `infra-mcp` não deveria conseguir se auto-modificar silenciosamente fora do caminho auditado do break-glass.
- Decisão em aberto, **não resolvida**: `evolution-api` tem senha de banco e API key em texto puro no `docker-compose.yml` (valor documentado — não acidental — em `vp-automations-hub/evolution-api/verticalparts-pv360.md`), e o mesmo valor está configurado também no `.env` do Node.js do `posvenda360.vpsistema.com` (Hostinger shared hosting). Rotacionar exige troca coordenada nos dois sistemas; operador optou por adiar essa decisão.
